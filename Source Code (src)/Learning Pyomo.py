...
T = range(96)                 # 96 time steps (example)
m.T = Set(initialize=T, ordered=True)

prices = {t: (30.0 if t < 48 else 80.0) for t in m.T}
...
print("Solved.")
print("charge[10] =", value(m.charge[10]))
print("discharge[10] =", value(m.discharge[10]))
print("soc[10] =", value(m.soc[10]))

print("charge[60] =", value(m.charge[60]))
print("discharge[60] =", value(m.discharge[60]))
print("soc[60] =", value(m.soc[60]))

profit = sum(value(m.price[t]) * (value(m.discharge[t]) - value(m.charge[t])) * PARAMS["dt"] for t in m.T)
...
results = pd.DataFrame(
    index=list(m.T),
    data={
        "price": [value(m.price[t]) for t in m.T],
        "charge_mw": [value(m.charge[t]) for t in m.T],
        "discharge_mw": [value(m.discharge[t]) for t in m.T],
        "soc_mwh": [value(m.soc[t]) for t in m.T],
        "r_up" : [value(m.r_up[t]) for t in m.T],
        "r_down" : [value(m.r_down[t]) for t in m.T],
        "res_up_price": [value(m.res_up_price[t]) for t in m.T],
        "res_down_price": [value(m.res_down_price[t]) for t in m.T],
    },
)
...
print("Gross energy margin (£):", results["gross_gbp"].sum())
print("Degradation cost (£):", results["penalty_gbp"].sum())
print("Net objective (£):", results["net_gbp"].sum())
print("Objective from Pyomo (£):", value(m.obj)) # error check, should be the same as net

# -----------------------------
# Breakdown Prints
# -----------------------------

print("Energy Revenue (£):", results["energy_gbp"].sum())
print("Reserve Revenue (£):", results["reserve_gbp"].sum())
print("Degradation Penalty (£):", results["penalty_gbp"].sum())
print("Net objective (£):", results["net_gbp"].sum())
...
'''
print('Model created')
print("Time set length:", len(list(m.T)))
print("First/last t:", list(m.T)[0], list(m.T)[-1])w

print("Time steps:", len(list(m.T)))
print("Example price t=0:", float(m.price[0]))
print("Example variables at t=0:")
print("  charge[0]   =", m.charge[0])
print("  discharge[0]=", m.discharge[0])
print("  soc[0]      =", m.soc[0])

print("Example constraints at t=0:")
print("  charge_cap[0]   =", m.charge_cap[0])
print("  discharge_cap[0]=", m.discharge_cap[0])
print("  soc_bounds[0]   =", m.soc_bounds[0])
print("Charge cap expr:", m.charge_cap[0].expr)
print("Discharge cap expr:", m.discharge_cap[0].expr)
print("SoC bounds lower:", m.soc_bounds[0].lower)
print("SoC bounds body :", m.soc_bounds[0].body)
print("SoC bounds upper:", m.soc_bounds[0].upper)

print("Objective:", m.obj.expr)
print("Initial SoC constraint:", m.soc_init.expr)
print("SoC balance expr t=0:", m.soc_balance[0].expr)
print("SoC balance expr t=94:", m.soc_balance[94].expr)
print("Is there a constraint at t=95?", 95 in m.soc_balance)
'''